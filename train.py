...
def make_PR_curves(
    ...
    for i in range(nSubsets):
        ...
        precision, recall, thr = precision_recall_curve(labels_flat[~is_missing],predictions_flat[~is_missing],
            sample_weight=np.concatenate(weights_repeated)[~is_missing] )
        
                
        # print(f"Max precision: {max(precision)}. Recall: {recall[precision.argmax()]} Threshold: {thr[precision.argmax()]}")
        # print(f"Max recall: {max(recall)}. Precision: {precision[recall.argmax()]}. Threshold: {thr[recall.argmax()]}")
        
        # with open("p_r_t.txt", 'w') as file:
        #     txt = "precision:\n"
        #     for p in precision:
        #         txt += str(p) + ","
        #     txt += "\n\n"
